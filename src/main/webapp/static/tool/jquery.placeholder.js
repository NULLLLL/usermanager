...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		...
		(function($) {
			// Opera Mini v7 doesn’t support placeholder although its DOM seems
			// to indicate so
			...
			if (isInputSupported && isTextareaSupported) {
				...
				hooks = {
					...
					'set' : function(element, value) {
						...
						if (value === '') {
							element.value = value;
							// Issue #56: Setting the placeholder causes
							// problems if the element continues to have focus.
				...
				$(function() {
					...
					$(document).delegate(
							...
							function() {
								...
								// get submitted
								var $inputs = $('.' + settings.customClass,
				...
				if (input.value === '') {
					if (input.type === 'password') {
						...
						$input = $input.removeAttr('id').hide().prevAll(
								'input[type="text"]:first').attr('id', id)
								.show();
